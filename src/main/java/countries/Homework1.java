...
public class Homework1 {
    ...
    public boolean streamPipeline1() {
        return countries.stream().anyMatch(a->a.getName().toLowerCase().contains("island"));
    }
    ...
    public Optional<String> streamPipeline2() {
        return countries.stream().map(a->a.getName()).filter(b->b.toLowerCase().contains("island")).findFirst();
    }
    ...
    public void streamPipeline4() {
        countries.stream().map(a->a.getPopulation()).sorted(Comparator.naturalOrder()).limit(10).forEach(System.out::println);
    }
